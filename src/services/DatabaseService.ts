...
class DatabaseService {
    ...
    async upsertMessage(messageData: MessageData): Promise<any> {
        try {
            ...
            let previousMessageText: string | null = null;

            if (isEdit) {
                const existingRows = await prisma.$queryRawUnsafe<Array<{ message: string | null }>>(
                    `SELECT message FROM messages WHERE id = $1 LIMIT 1`,
                    messageData.id
                );
    ...
    async upsertReaction(
        ...
        try {
            ...
            const result = await prisma.message_reactions.create({
     ...
     */
    async getGroupName(chatId: string): Promise<string | null> {
        const sanitizedId = sanitizeChatId(chatId);
