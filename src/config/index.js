...
const envPaths = [
    path_1.default.join(__dirname, '..', '..', '.env'), // backend/.env (from src/config)
    path_1.default.join(process.cwd(), 'backend', '.env'), // ./backend/.env (from root)
    path_1.default.join(process.cwd(), '.env'), // ./.env (from backend folder)
];
let envFound = false;
...
if (!envFound) {
    (0, dotenv_1.config)(); // Fallback to default behavior
}
// Validation helper
...
exports.CONFIG = {
    ...
    SSL: {
        ...
        BASE_PATH: process.env.SSL_BASE_PATH || path_1.default.join(__dirname, '..', '..', 'ssl'),
    },
    // CORS
    ...
    // Paths
    PATHS: {
        IMAGES: path_1.default.join(__dirname, '..', '..', 'imgs'),
        VIDEOS: path_1.default.join(__dirname, '..', '..', 'video'),
        AUDIO: path_1.default.join(__dirname, '..', '..', 'audio'),
        DOCUMENTS: path_1.default.join(__dirname, '..', '..', 'docs'),
    },
    // Timezone
...
function validateConfig() {
    const errors = [];
    if (!exports.CONFIG.WUZAPI.BASE_URL.startsWith('http')) {
        errors.push('WUZAPI must be a valid HTTP(S) URL');
    }
    if (exports.CONFIG.UPLOAD.MAX_FILE_SIZE_MB > 200) {
