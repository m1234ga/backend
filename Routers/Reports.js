...
router.get('/chat-tags', async (req, res) => {
    try {
        ...
        const totalsResult = await DBConnection_1.default.query(`
SELECT
  ...
  COUNT(DISTINCT ct."tagId")::int as "uniqueTags"
    ...
    `, [applyCreatorFilter, createdByCandidates, startDate]);
        const topTagsResult = await DBConnection_1.default.query(`
SELECT
  ...
  MAX(ct."creationDate") as "lastTaggedAt"
    ...
    `, [applyCreatorFilter, createdByCandidates, startDate]);
        const taggedChatsResult = await DBConnection_1.default.query(`
SELECT
  c.id as "chatId",
...
router.get('/volume', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const chatsQuery = await DBConnection_1.default.query(`
      SELECT
        COUNT(DISTINCT c.id) as "totalChats",
        COUNT(DISTINCT CASE WHEN csd.status = 'open' THEN c.id END) as "openChats",
      ...
      WHERE c."lastMessageTime" >= $1
    `, [startDate]);
        const msgsQuery = await DBConnection_1.default.query(`
      SELECT
        COUNT(CASE WHEN "isFromMe" = true THEN 1 END) as "sentMessages",
        COUNT(CASE WHEN "isFromMe" = false THEN 1 END) as "receivedMessages"
      FROM messages
    ...
    `, [startDate]);
        ...
        const prevChatsQuery = await DBConnection_1.default.query(`
    ...
    `, [prevStartDate, startDate]);
        ...
        const trendsQuery = await DBConnection_1.default.query(`
      SELECT 
...
router.get('/performance', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const agentsQuery = await DBConnection_1.default.query(`
    ...
    `, [startDate]);
        const reassignmentQuery = await DBConnection_1.default.query(`
...
router.get('/tags', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const tagsQuery = await DBConnection_1.default.query(`
      SELECT 
        t."tagId"::text as "tagId",
        t."tagName",
...
router.get('/peak-hours', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const hourlyQuery = await DBConnection_1.default.query(`
      SELECT 
      ...
      ORDER BY hour ASC
    `, [startDate]);
        const weekdayQuery = await DBConnection_1.default.query(`
      SELECT 
...
router.get('/sla', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const slaTargets = await DBConnection_1.default.query(`SELECT * FROM sla_targets`);
        ...
        const slaQuery = await DBConnection_1.default.query(`
      WITH chat_slas AS (
        SELECT 
          c.id,
          c."assignedTo",
    ...
    `, [startDate]);
        ...
        slaQuery.rows.forEach(row => {
            ...
            if (row.frt !== null) {
                ...
                frtCount++;
                if (parseFloat(row.frt) > frtTarget)
                    isBreached = true;
            }
            if (row.art !== null) {
                totalArt += parseFloat(row.art);
                artCount++;
                if (parseFloat(row.art) > artTarget)
                    isBreached = true;
            }
            if (isBreached) {
                breachedSla++;
...
router.get('/workload', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const queueQuery = await DBConnection_1.default.query(`
      SELECT
      ...
      WHERE "lastMessageTime" >= $1
    `, [startDate]);
        const workloadQuery = await DBConnection_1.default.query(`
      SELECT 
        "assignedTo", 
...
router.get('/customers', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const activityQuery = await DBConnection_1.default.query(`
      SELECT
      ...
      WHERE c."lastMessageTime" >= $1
    `, [startDate]);
        const topCustomersQuery = await DBConnection_1.default.query(`
      SELECT 
        c.name,
...
router.get('/transfers', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const transfersQuery = await DBConnection_1.default.query(`
      SELECT 
        "chatId",
    ...
    `, [startDate]);
        ...
        const byEmployeeQuery = await DBConnection_1.default.query(`
...
router.get('/pending', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const pendingQuery = await DBConnection_1.default.query(`
      SELECT
...
router.get('/csat', async (req, res) => {
    try {
        const { dateRange = '7days' } = req.query;
        const startDate = resolveStartDate(dateRange);
        const csatQuery = await DBConnection_1.default.query(`
    ...
    `, [startDate]);
        ...
        const agentCsatQuery = await DBConnection_1.default.query(`
      ...
      LIMIT 10
    `, [startDate]);
        const trendQuery = await DBConnection_1.default.query(`
...
router.get('/executive', async (req, res) => {
    try {
        ...
        const kpiQuery = await DBConnection_1.default.query(`
      SELECT 
...
router.get('/audit', async (req, res) => {
    try {
        ...
        const auditQuery = await DBConnection_1.default.query(`
      SELECT 
        al.id,
    ...
    `, [parseInt(limit), offset]);
        ...
        if (logs.length === 0) {
            const chatTagsActivity = await DBConnection_1.default.query(`
        SELECT
