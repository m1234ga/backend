...
export class ChatStatusManager {
  ...
  static async getCurrentStatus(chatId: string) {
    const query = `
  ...
  static async getStatusHistory(chatId: string) {
    const query = `
      SELECT status, changed_at, changed_by, reason, notes
      FROM chat_status_details
      WHERE chat_id = $1
