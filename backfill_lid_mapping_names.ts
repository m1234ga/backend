...
function stripJidSuffixExpr(column: string): string {
...
async function dryRun(): Promise<void> {
  ...
  const preview = await prisma.$queryRawUnsafe<Array<{
    ...
    old_push_name: string | null;
    new_push_name: string | null;
...
async function applyBackfill(): Promise<void> {
  const updatedRows = await prisma.$queryRawUnsafe<Array<{ lid: string }>>(
    `
    WITH normalized_contacts AS (
      SELECT
        ${stripJidSuffixExpr('their_jid')} AS jid_bare,
